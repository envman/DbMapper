...
-(void)fetch
{
    ...
    if ([result next])
    {
        for (NSUInteger i = 0; i < _mappings.count; i++)
        {
            ...
            {
                [self.mappedObject setValue:[result stringForColumn:mapping.column] forKey:mapping.property];
            }
            else if ([DataHelper isNumberType:propertyType])
            {
                [self.mappedObject setValue:@([result intForColumn:mapping.column]) forKey:mapping.property];
            }
            else
            {
                id subObject = [[NSClassFromString(propertyType) alloc] init];

                if ([subObject isSubclassOfClass:[self class]])
                {
                    Mappable *mappable = subObject;
                    [mappable fetchWithId:(NSUInteger) [result intForColumn:mapping.column]];

                    [self.mappedObject setValue:subObject forKey:mapping.property];
                }
            }
        }
    }
...
- (void)update
{
    ...
    for (NSUInteger i = 0; i < _mappings.count; i++)
    {
        ...
        id value = [self.mappedObject valueForKey:mapping.property];
